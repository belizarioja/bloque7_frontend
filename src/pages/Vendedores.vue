          ...
          <q-card>
            <q-card-section style="display:block; height: 48px;">
              ...
              <div style="float: right;">
                <q-icon class="iconApp" name="manage_accounts" color="primary" style="font-size: x-large;" />
              </div>
              <div style="float: right;">
                <q-icon class="iconApp" name="disabled_by_default" color="negative" style="font-size: x-large;margin-right:10px;" />
              </div>
            </q-card-section>
          </q-card>
        </div>
  ...
  methods: {
    ...
    async listarVendedores () {
      ...
      for (const i in datos) {
        const item = datos[i]
        const obj = {}
        obj.id = item.VENDV_IDVENDEDOR
        obj.nombre = item.VENDV_NOMBRE
